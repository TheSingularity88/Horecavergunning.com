import { FileCheck, Scale, TrendingUp, HardHat } from 'lucide-react';

export function Services() {
  const services = [
    {
      title: "Permits & Bibob",
      desc: "Fast-track your liquor and terrace permits. We manage the complex Bibob screening process so you open on time.",
      icon: FileCheck
    },
    {
      title: "Legal & Contracts",
      desc: "Watertight employment contracts, rental agreements, and terms & conditions tailored for hospitality.",
      icon: Scale
    },
    {
      title: "Finance & Tax",
      desc: "Real-time insight into your margins, tax filings, and annual accounts. Optimized for Horeca businesses.",
      icon: TrendingUp
    },
    {
      title: "Project Management",
      desc: "Starting fresh or renovating? We coordinate contractors and regulations for smooth takeovers and builds.",
      icon: HardHat
    }
  ];

  return (
    <section id="services" className="py-24 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <h2 className="text-3xl font-bold text-slate-900 mb-4">Everything You Need to Run a Compliant Venue</h2>
          <p className="text-slate-600">From the moment you sign the lease to your daily operations, we've got you covered.</p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, i) => (
            <div key={i} className="group p-6 rounded-2xl border border-slate-100 hover:border-amber-200 hover:shadow-lg transition-all duration-300 bg-white">
              <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center mb-6 group-hover:bg-amber-500 transition-colors">
                <service.icon className="w-6 h-6 text-slate-600 group-hover:text-white transition-colors" />
              </div>
              <h3 className="text-lg font-bold text-slate-900 mb-2">{service.title}</h3>
              <p className="text-slate-600 text-sm leading-relaxed">{service.desc}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
