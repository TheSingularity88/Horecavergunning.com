  ...
  return (
    <div className="fixed top-0 w-full z-50">
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200">
        ...
        {/* Mobile Navbar */}
        <div className="md:hidden container mx-auto px-4 h-16 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2">
          {/* Left: Hamburger Menu */}
          <button 
            onClick={() => setIsMobileMenuOpen(true)}
            className="p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-md transition-colors justify-self-start"
          ...
          <div className="flex items-center gap-1 -mr-2 z-20 relative justify-self-end">
                ...
                {isUserMenuOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute top-full right-0 mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl z-20 origin-top-right overflow-hidden"
                    >
                      <div className="p-3 grid gap-2">
                        <Link 
                          href="/login" 
                          onClick={() => setIsUserMenuOpen(false)}
                          className="flex items-center justify-center rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                        >
                          {t.navbar.login}
                        </Link>
                        <Link 
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 bottom-0 w-[85%] max-w-sm bg-white z-[60] shadow-xl md:hidden flex flex-col"
            >
